...
export async function POST(req: Request, ctx: { params: { id: string } }) {
  try {
    ...
    try {
      ...
      await auditLog("admin_commitment_success_ok", { commitmentId: id, signature, amountLamports });
      ...
      await auditLog("admin_commitment_success_error", { commitmentId: id, error: getSafeErrorMessage(e) });
      await releaseResolutionClaim(id);
      return NextResponse.json({ error: getSafeErrorMessage(e) }, { status: 500 });
    }
  } catch (e) {
    await auditLog("admin_commitment_success_error", { commitmentId: ctx.params.id, error: getSafeErrorMessage(e) });
    return NextResponse.json({ error: getSafeErrorMessage(e) }, { status: 500 });
  }
}
