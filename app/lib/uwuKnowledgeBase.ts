...
What it does:
- Helps you send SOL (and some SPL flows server-side) using a short chain of ephemeral burner wallets (typically 2-5 hops).
...
- You sign a funding transaction from your connected wallet to the first burner address.
- The server detects funding and then routes the funds hop-by-hop.
- Burner wallets are Privy-managed; private keys are not exposed to the browser.
